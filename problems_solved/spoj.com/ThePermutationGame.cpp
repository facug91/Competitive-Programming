/*
        By: facug91
        From: https://www.spoj.com/problems/TPGAME/
        Name: The Permutation Game
        Date: 03/12/2022
...
int binary_search(int val) {
	...
	while (high - low > 1) {
		int mid = (low + high) / 2;
		int aux = ft.sum(mid);
		if (aux < val) low = mid + 1;
		else if (aux >= val) high = mid;
	}
	if (ft.sum(low) == val) return low;
	else return high;
}
