/*
        By: facug91
        From: https://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=788
...
bool search (ll p) {
	if (p >= n) return true;
	else if (search(p*9ll) || search(p*2ll)) return false;
	return true;
}
...
int main () {
	...
	while (cin>>n) {
		if (!search(1ll)) cout<<"Stan wins."<<endl;
