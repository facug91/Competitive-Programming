...
bool check (ll fk) {
	// c es el valor que representa la posición j, considerando que el siguiente valor a i es fk
	...
	// retorna verdadero si con este fk pudo alcanzar (o superar) fj
	return fj <= c;
}
...
int main () {
	...
	if (n == i) cout<<fi<<endl;
	...
	else if (n < i) { // si hay que ir para atrás
		while (n < i) {
			fj = fk;
			fk = fi;
			fi = fj - fk;
		...
		cout<<fi<<endl;
	} else /* if (n > j) */ { // si hay que ir para adelante
		while (n > j) {
			fi = fk;
			fk = fj;
			fj = fi + fk;
