/*
	By: facug91
	From: https://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=3911
	Name: Secret Word
	Date: 27/06/2016
*/
...
int main () {
	...
	while (tc--) {
		...
		int ans = t.length()+1;
		for (i=t.length()+1; i<z.size(); i++) if (z[ans] < z[i]) ans = i;
		//for (i=0; i<z.size(); i++) cerr<<z[i]<<", ";
		//cerr<<endl;
		//cerr<<"ans "<<ans<<endl;
		for (i=ans+z[ans]-1; i>=ans; i--) cout<<s[i];
		cout<<endl;
	}
